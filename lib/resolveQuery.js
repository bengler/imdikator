...
function listify(arr, options = {}) {
  const defaults = {oxfordComma: false, conjunction: 'and'}
...
export default function resolveQuery(region, srcQuery, headerGroups) {
  ...
  if (!possibleValues) {
    ...
    throw new Error(
      `Found no header group with values for the combination of ${listify([regionHeaderKey, ...dimNames])}]. `
    + `Possible combinations for ${regionHeaderKey} are ${listify(combinationsForHeaderKey, {conjunction: 'or'})}`
    )
  }
