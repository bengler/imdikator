...
import {findHeaderGroupForQuery} from './queryUtil'
...
export function constrainQuery(query, querySpec, config = {}) {
  ...
  if (invalidRegions.length > 0) {
    ...
    result.operations.push({
      ...
      description: `Ikke mulig å velge regionen(e) ${invalidRegions.join(', ')}.`
    })
  }
  ...
  result.query.dimensions = query.dimensions
    .map(queryDim => {
      ...
      const dimensionsConfig = (config.dimensions || {})[queryDim.name] || {}
      ...
      let variables = queryDim.variables
      if (!variables || variables == 'all') {
        variables = dimensionsConfig.include
      }

      if (variables && !dimFromSpec.choices.some(valuesEqual(variables))) {
        ...
        result.operations.push({
          dimension: queryDim.name,
          description: `Ugyldig verdi ${JSON.stringify(variables)
            }. Må være en av ${
...
export function getQuerySpec(query, {tab, headerGroups, chart, config = {}}) {

  const dimensionsConfig = config.dimensions || {}
  const currentHeaderGroup = findHeaderGroupForQuery(query, headerGroups)

  const validRegions = uniq(headerGroups.reduce((acc, headerGroup) => {
    const regionKey = ['kommuneNr', 'fylkeNr', 'bydelNr', 'naringsregionNr'].find(key => headerGroup.hasOwnProperty(key))
    const regionType = getRegionTypeFromHeaderKey(regionKey)
    return acc.concat(headerGroup[regionKey].map(regionCode => prefixify(regionType, regionCode)))
  }, []))
  ...
  const year = {
    ...
    choices: tab.name === 'chronological' ? ['all'] : currentHeaderGroup.aar.map(y => [y])
  }
  const unit = {
    name: 'unit',
    fixed: true,
    ...
    locked: false,
    choices: currentHeaderGroup.enhet.map(un => [un])
  }
  ...
  return [comparisonRegions, year, unit, ...freeDimensions]

  function transformPyramidChartDimensions(dimension, i) {

    const dimensionConfig = dimensionsConfig[dimension.name] || {}
    const excludeFromChart = dimensionConfig.excludeFromChart === true
    const possibleValues = dimensionConfig.include || currentHeaderGroup[dimension.name]
  ...
  function transformDefaultChartDimensions(dimension, i) {
    ...
    const possibleAggregateValue = !config.isLopsided && possibleValues.find(val => val === 'alle')
    ...
    return {
      locked,
      hidden: dimensionConfig.hidden === true,
      constrained: constrained,
      name: dimension.name,
      choices
    }
