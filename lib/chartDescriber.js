...
const debug = _debug.create('chartDescription')
...
function chartLabel(cardName, unit) {
  const desc = cardDescriptions.find(card => card.cardName == cardName) || {}
  return desc ? desc.unitDescriptions[unit] : `[missing chart description ${cardName}]`
}
...
export function queryToOptions(query, cardName, headerGroup, allRegions) {
  debug('query', query)
  //console.log('query', util.inspect(query, {depth: null}))
  const opts = {}

  // showing
  opts.showing = chartLabel(cardName, query.unit)
  ...
  query.dimensions.forEach(dimension => {
    ...
    if (dimension.variables.length > 1) {
      dimension.variables.forEach(variable => {
        opts.groupedBy.push(dimensionLabelTitle(dimension.name, variable).toLowerCase())
      })
    }
  })
  return opts
