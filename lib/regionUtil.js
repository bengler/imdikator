...
export function prefixify(regionType, regionCode) {
  ...
  return TYPE_TO_PREFIX[regionCode] + regionCode
}
...
export function comparableRegions(region, regions) {
  ...
  switch (region.type) {
    ...
    case 'commerceRegion':
      //const commerceRegions = regions.filter(thing => thing.type == 'commerceRegion' && thing.code != region.code && thing.countyCode == region.countyCode)

      // TODO: figure out county code for commerceRegion
      //console.log('similar commerceRegions', commerceRegions)
      return []
