...
import toVismaCompareQuery from './toVismaCompareQuery'
...
export function create({baseUrl, adapter} = {}) {
  ...
  function query(q) {
    if ((q.comparisonRegions || []).length > 0) {
      return queryVisma('data/compare', toVismaCompareQuery(q))
    }
    // TODO: Move over to data/compare for all queries
    return queryVisma('data/query', toVismaQuery(q))
  }

  function queryVisma(endpoint, vismaQuery) {
    return Promise.resolve().then(() => {
      const url = Url.format({
        protocol: parsedBaseUrl.protocol,
        host: parsedBaseUrl.host,
      ...
      return adapter
        .post(Url.format(url), toQueryParams(vismaQuery))
