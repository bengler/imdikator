...
export default function csvDimensionsBuilder(dimensions) {
  ...
  const result = dimensions.reduce((allDimensions, dimension) => {
    if (!allDimensions.dimension) {
      // Ensure variable is array
      let dimensionVariables = []
      if (Array.isArray(dimension.variables)) {
        dimensionVariables = dimension.variables
      } else {
        dimensionVariables = []
        dimensionVariables.push(dimension.variables)
      }

      allDimensions[dimension.name] = {
        label: dimensionLabelTitle(dimension.name),
        values: dimensionVariables.reduce((allVariables, variable) => {
