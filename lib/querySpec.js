...
export function getQuerySpec(query, {tab, headerGroup, chart, allRegions, config = {}}) {
  ...
  const comparisonRegions = {
    ...
    choices: allRegions.map(reg => reg.prefixedCode)
  }
  const year = {
    name: 'year',
    fixed: true,
  ...
  function transformPyramidChartDimensions(dimension, i) {
    ...
    const possibleValues = dimensionConfig.include || headerGroup[dimension.name]
