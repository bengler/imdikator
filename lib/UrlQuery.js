...
// Because IIS doesn't handle + in urls, either raw or encoded (╯°□°）╯︵ ┻━┻
// Replacing regular plus sign with full-width plus	(U+FF0B)
function encodeForIIS(value) {
  return value.replace(/\+/g, '＋')
}

function decodeForIIS(value) {
  return value.replace(/＋/g, '+')
}
...
function stringify(query) {
  ...
  return Object.keys(stringified).map(key => {
    return `${key}=${encodeURIComponent(encodeForIIS(stringified[key]))}`
  }).join(';')
}
...
function parseDimension(name, value) {
  return [name, decodeForIIS(decodeURIComponent(value))]
}
