...
export function allCounties(regions) {
  const unsorted = regions.filter(region => (region.type == 'county') && region.prefixedCode != 'F00' && !region.name.includes('Uoppgitt'))
...
export function comparableRegions(region, regions) {
  switch (region.type) {
    ...
    case 'borough':
      return regions.filter(thing => {
        const isInSameMunicipality = thing.municipalityCode == region.municipalityCode
        const isNotSame = thing.code != region.code
        return thing.type == 'borough' && isNotSame && isInSameMunicipality && !thing.name.includes('Uoppgitt')
      })

    // commerceRegion, show other commerceRegions in same county
    case 'commerceRegion':
      return regions.filter(thing => {
        const isInSameCounty = thing.countyCode == region.countyCode
        const isNotSame = thing.code != region.code
        return thing.type == 'commerceRegion' && isNotSame && isInSameCounty
      })
    ...
    case 'county':
      return regions.filter(thing => {
        const isNotSame = thing.code != region.code
        return thing.type == 'county' && isNotSame && thing.code != '00' && !thing.name.includes('Uoppgitt')
      })
