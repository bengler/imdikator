...
export function queryResultPresenter(query, result, config) {
  ...
  if (query.dimensions && query.dimensions.some(dim => dim.name == 'aar' && dim.variables.length == 1)) {
    dimensionsToRemove.push('aar')
  }

  const dimensions = (query.dimensions || []).filter(dim => !dimensionsToRemove.includes(dim.name)).map(dim => dim.name)
