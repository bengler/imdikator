...
const CSV_FILE_KOMMUNER_BYDELER =
  './import/regioninndeling-kommuner-bydeler.csv';
...
const naeringsregioner = parsedRegions
  ...
  .map(
    pickKeys('Næringsregionnr', 'Næringsregion_navn', 'Kommunenr', 'Fylkenr')
  )
  .map(
    renameKeys({
      Næringsregionnr: 'code',
      Næringsregion_navn: 'name',
...
naeringsregioner.subscribe(res =>
  log('Wrote %s næringsregioner to %s', res.entries.length, res.file)
);
kommuner.subscribe(res =>
  log('Wrote %s kommuner to %s', res.entries.length, res.file)
);
bydeler.subscribe(res =>
  log('Wrote %s bydeler to %s', res.entries.length, res.file)
);
fylker.subscribe(res =>
  log('Wrote %s fylker to %s', res.entries.length, res.file)
);

// A few helper functions
function csvToObjects(file) {
  const rows = RxNode.fromReadableStream(fs.createReadStream(file).pipe(csv()));
  return rows.skip(1).withLatestFrom(rows.take(1), (row, header) => {
