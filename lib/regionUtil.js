...
export function comparableMunicipalities(region, regions) {
  ...
  const matching = similarMunicipalities.find(thing => thing.code == region.code)
  assert(matching, `No similar municipalities for region ${region.name} asdf`)
  return matching.similar.map(code => {
