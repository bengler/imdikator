...
function languagify(dimension) {
  ...
  // Temporary fix to avoid uppercased first letters in the middel of the sentence when figures are delimited
  // This can not be done with all dimensions since some are countrynames and the like.
  if (
    dimension.name == 'avslstat4'
    || dimension.name == 'forsteBosettingskommune'
    || dimension.name == 'utd6'
    || dimension.name == 'undervisningGrskole'
    || dimension.name == 'innvkat3'
    ) {
    subject = downcaseFirstLetter(subject)
  }
