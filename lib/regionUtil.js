...
function allRegionsOfType(type, skip, regions) {
  const unsorted = regions.filter(region => {
    const hasType = region.type == type;
    const dontSkip = region.prefixedCode != skip;
    const notListed = region.name && !region.name.includes('Uoppgitt');

    return hasType && dontSkip && notListed;
  });

  return unsorted.sort((reg1, reg2) => {
    const truth = reg1.name < reg2.name ? -1 : 1;
    return truth;
  });
}
...
const HEADER_KEY_TO_REGION_TYPE_MAPPING = mirror(
  REGION_TYPE_TO_HEADER_KEY_MAPPING
);
...
export function prefixify(regionType, regionCode) {
  assert(
    REGION_TYPE_TO_PREFIX[regionType],
    `No prefix registered for region type ${regionType}`
  );
...
export function getHeaderKey(region) {
  assert(
    REGION_TYPE_TO_HEADER_KEY_MAPPING[region.type],
    `No mapping from region type ${region.type} to header key found`
  );
...
export function getHeaderKeyForRegionCode(regionCode) {
  ...
  assert(
    headerKey,
    `Unable to find header key for ${regionCode} (prefix ${prefix})`
  );
  return headerKey;
}
...
export function parentRegionByType(parentType, parentCode, regions) {
  return regions.find(
    region => region.code == parentCode && parentType == region.type
  );
}

export function isSimilarRegion(region) {
  const fromSimilarMapping =
    region.type === 'municipality' &&
    similarMunicipalities.find(candidate => candidate.code == region.code);
  ...
  return function(otherRegion) {
    ...
    switch (region.type) {
      ...
      case 'borough':
        return (
          otherRegion.municipalityCode == region.municipalityCode &&
          !otherRegion.name.includes('Uoppgitt bydel')
        );
      ...
      case 'municipality': {
        if (!fromSimilarMapping) {
          return false;
        }
        return fromSimilarMapping.similar.some(
          code => otherRegion.code === code
        );
      }

      default:
        throw new Error(
          `Missing region type when looking for similar regions of ${region}`
        );
    }
  };
}
...
// Called from DownloadWidget.jsx where the user selects content for CSV file to download
...
export function getPageTitle(region, prefix = null) {
  let title = 'Integreringen i ';
  if (prefix) {
    title = `${prefix} integreringen i`;
  }
  if (region.prefixedCode == 'F00') {
    return `${title} Norge`;
  }
  return `${title} ${region.name} ${_t(region.type)}`;
}

export function getPageIngress(region) {
  if (region.prefixedCode == 'F00') {
    return 'Tall og statistikk over integreringen i Norge';
  }
  return `Tall og statistikk over integreringen i ${_t(`the-${region.type}`)}`;
}
