...
export function regionByCode(code, type, regions) {
  console.log('deprecation warning: regionByCode(code, type, regions), use regionByPrefixedCode(prefixedCode, regions) instead') // eslint-disable-line no-console
  return regions.find(region => (region.code == code) && region.type == type)
}

export function regionByPrefixedCode(prefixedCode, regions) {
  return regions.find(region => region.prefixedCode.toLowerCase() == prefixedCode.toLowerCase())
}
